"""``omnigent login`` accounts-auth detection under a proxy environment.

An accounts-mode server answers unauthenticated ``GET /v1/me`` with
``401 {"user_id": null, "login_url": "/login"}``, and ``omnigent login``
routes on that payload to the username/password flow. But the probe (and
the OIDC fallback POST) run httpx with environment defaults, so when the
machine carries ``HTTP(S)_PROXY`` and the server host is not in
``NO_PROXY``, the proxy's own answer (e.g. an unconditional 403) replaces
the server's 401 payload. The login command then silently falls into the
OIDC ticket flow and dies on ``POST /auth/cli-login`` with the misleading
"Is the server running with OMNIGENT_AUTH_PROVIDER=oidc?" error, even
though the accounts server is reachable — and ``omnigent diagnose``
(which probes with ``trust_env=False``) correctly reports accounts auth
on the same machine.

This test drives the real user journey: a real ``omnigent server``
subprocess with accounts auth, a deny-all HTTP proxy configured through
the environment, and the real ``omnigent login`` command on a PTY.
Loopback traffic must never be routed through a proxy (see
``is_loopback_url`` / ``_trust_env_for``), so login must still detect
accounts auth, prompt for credentials, and complete.
...
def _await_health(base_url: str, log_path: Path) -> None:
    ...
    :param base_url: Server base URL, e.g. ``"http://127.0.0.1:58123"``.
...
def deny_all_proxy() -> Iterator[str]:
    ...
    routes the login traffic through the environment proxy sees this 403
    instead of the omnigent server's responses.
    """
...
def accounts_server(tmp_path: Path) -> Iterator[str]:
    ...
    The first-boot admin is provisioned headlessly via
    ``OMNIGENT_ACCOUNTS_INIT_ADMIN_USERNAME`` / ``..._PASSWORD`` so the
    login under test has a real credential to authenticate with.
    ...
    # An ambient OIDC issuer would otherwise flip the auth-source switch.
...
def test_login_detects_accounts_auth_despite_proxy_env(
    ...
    try:
        ...
        if index != 0:
            pytest.fail(
                "omnigent login misdetected the accounts server and fell "
                "into the OIDC ticket flow instead of prompting for "
                f"username/password. Output:\n{child.before}"
            )
